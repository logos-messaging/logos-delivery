...
LOGOSCORE="${LOGOSCORE:-$(nix build github:logos-co/logos-liblogos --override-input logos-cpp-sdk github:logos-co/logos-cpp-sdk/a4bd66c --no-link --print-out-paths)/bin/logoscore}"
...
EOF
    ...
    for j in $(seq 1 90); do
        ...
        if ! kill -0 "${INSTANCE_PIDS[$i]}" 2>/dev/null; then
            echo "  ERROR: Node $i exited. Last log:"
            tail -10 "$LOG_FILE"
            exit 1
        fi
        sleep 1
    ...
    # After bootstrap node, brief pause for P2P to start listening
    if [ "$i" -eq 0 ]; then
        sleep 3
    fi
done
