...
#   every path below starts with a target name
#   (`logosdeliverynode-windows-x86_64/bin/...`, never `bin/...`). `run` is a
#   wrapper on PATH: wine on the Linux pre-filter leg, a direct exec on the
#   real windows-latest leg, so this one file serves both. It refuses a path
#   that does not exist and refuses a command that exits 0 having printed
#   nothing, and on failure reports the exit code, both streams and what
