...
proc registerFFIEventListeners(self: LogosDelivery): Result[void, string] =
  ...
  ServiceDiscoveryHostRequest.listen(
    self.waku.brokerCtx,
    proc(event: ServiceDiscoveryHostRequest) {.async: (raises: []).} =
    ...
    chronicles.error "ServiceDiscoveryHostRequest.listen failed", err = $error
    return err("ServiceDiscoveryHostRequest.listen failed: " & $error)
...
proc teardownFFIEventScope(self: LogosDelivery) {.async.} =
  ...
  await ServiceDiscoveryHostRequest.dropAllListeners(self.waku.brokerCtx)
