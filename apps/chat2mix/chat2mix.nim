...
proc processInput(rfd: AsyncFD, rng: crypto.Rng) {.async.} =
  ...
  let node = block:
    ...
    let resolvedNat = await resolveNatStrategy(natStrategy)
    builder.withNatConfig(natConfig(resolvedNat))
  ...
  (
    await node.mountMix(
      conf.clusterId, mixPrivKey, conf.mixnodes, Opt.some(conf.rlnUserMessageLimit)
    )
  ).isOkOr:
