...
## External service discovery driven through the real C ABI, the way the
## logos-delivery-module hosts it: the driver `dlopen`s `liblogosdelivery`,
...
import std/[atomics, base64, dynlib, json, os, strutils]
import testutils/unittests

const
  RetOk = 0
  EventName = "onServiceDiscoveryRequest"
  SvcKey = "svc:/logos/delivery"
...
proc nodeConfig(): string =
  $(
    %*{
      ...
      "messagingOverrides": {
        "log-level": "INFO",
        "local-storage-path": getTempDir() / "ld-ffi-service-discovery",
        "tcp-port": "60110",
...
suite "External service discovery over the C ABI":
  test "a host answering events brings the node up and advertises it":
    let api = loadApi()
    ...
    check requirements["externalServiceDiscovery"].getBool()
    ...
    for req in host.requests:
      verbs.add(req["verb"].getStr())
      check:
        req["eventType"].getStr() == "service_discovery_request"
        req["timeoutMs"].getInt() > 0
    check:
      verbs[0] == "start"
      "registerInterest" in verbs
      "startAdvertising" in verbs

    for req in host.requests:
      if req["verb"].getStr() == "startAdvertising":
        check:
          req["key"].getStr() == SvcKey
          base64.decode(req["record"].getStr()).len > 0
