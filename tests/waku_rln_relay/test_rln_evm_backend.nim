...
# `setupRlnEvmBackend` flow (fund a fresh account + mint tokens + approve
...
suite "Onchain group manager":
  ...
  setup:
    if not anvilStarted:
      ...
      baselineSnapshotId = waitFor takeEvmSnapshot()
      manager = buildRlnEvmBackend(fundedPrivateKey)
  ...
  test "updateRecentRoots: appends new on-chain roots from contract cache":
    # Verify that the rln_evm_backend list of valid roots is updated correctly from the recent roots
  ...
  test "register: should guard against uninitialized state":
    ...
    check:
      res.isErr()
      res.error == "RlnEvmBackend is not initialized"
  ...
  test "validateRoot: should reject bad root":
    ...
    # chunk[0] becomes the MSB after reversal in rln_evm_backend; must be < 0x30
    for i in 0 ..< 20:
  ...
  test "proof roundtrip: generateRlnProofWithWitness -> verifyRlnProof":
    ...
    # Build RLNWitnessInput the same way rln_evm_backend.generateProof does.
