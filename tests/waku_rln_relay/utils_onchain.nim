...
proc setupRlnEvm*(
...
): Future[RlnEvmGroupManager] {.async.} =
  ...
  ## 2. Run setupRlnEvm with deployContracts=true to deploy contracts
  ...
  ## 3. Call setupRlnEvm with deployContracts=false
  ...
  let manager = RlnEvmGroupManager(
...
proc buildRlnEvm*(
  ...
  ## Constructs an RlnEvmGroupManager pointing at the cached RLN proxy contract
  ...
  ## token-approved account (e.g. via a prior `setupRlnEvm` followed
  ## by `takeEvmSnapshot`). Each call returns a fresh RLN instance.
  let rlnInstanceRes = createRlnInstance()
  check:
    rlnInstanceRes.isOk()
  return RlnEvmGroupManager(
    ethClientUrls: @[ethClientUrl],
