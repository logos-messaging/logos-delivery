...
type
  ...
  ReceivedMessageRecord* = object
    seq*: uint64
      ## position in the node's received stream, from 1; a gap is evicted records
    messageHash*: string
...
proc readValue*(
  ...
  value = ReceivedMessageRecord(
    seq: seq, messageHash: messageHash, message: message, source: source.get()
  )
