...
type
  ...
  ReliableChannel* = ref object
    ...
    encryption: ChannelEncryptionRegistry ## shared with the owning manager
...
proc encryptWith(
    crypto: Opt[ChannelCrypto], plaintext: seq[byte]
): Future[Result[seq[byte], string]] {.async: (raises: []).} =
  ## Fails closed: a registered cipher that errors aborts the message
  ## rather than letting plaintext reach the wire.
  if crypto.isNone():
    return ok(plaintext)
  return await crypto.unsafeGet().encryptFn()(plaintext)

proc decryptWith(
    crypto: Opt[ChannelCrypto], ciphertext: seq[byte]
): Future[Result[seq[byte], string]] {.async: (raises: []).} =
  if crypto.isNone():
    return ok(ciphertext)
  return await crypto.unsafeGet().decryptFn()(ciphertext)
...
proc send*(
  ...
  ## half-wrapped send leaves orphans that SDS will retransmit on its own
  ## and that later messages reference. Doing all the encryption first means
  ## a cipher failure aborts with nothing wrapped and nothing dispatched.
  var encryptedSegments: seq[seq[byte]]
  for segmentBytes in segments:
    ## The ciphertext becomes the SDS `content` field.
    let encrypted = (await crypto.encryptWith(segmentBytes)).valueOr:
...
proc onMessageReceived(
  ...
  ## Only the acquisition can raise (CancelledError); everything below is
  ## `raises: []`, so the try stays scoped to exactly the acquire.
  try:
  ...
  for item in deliverables:
    ...
    let plaintext = (await crypto.decryptWith(item.content)).valueOr:
      ChannelMessageLostEvent.emit(
        self.brokerCtx,
        ChannelMessageLostEvent(
          channelId: self.channelId,
          payloadHash: @[], ## unknowable: the payload never decrypted
          reason: "decryption failed: " & error & " (messageHash: " & messageHash & ")",
        ),
      )
      continue

    self.reportReceived(SdsDeliverable(content: plaintext, senderId: item.senderId))
