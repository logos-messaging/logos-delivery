...
proc stop*(self: ReliableChannelManager) {.async.} =
  ...
  ## registered ciphers do not. Note this drops the registrations without
  ## draining sends that already captured one, so it is not by itself a safe
  ## point for a caller to free what its closures captured.
  for chn in self.channels.values:
    await chn.stop()
  self.channels.clear()
