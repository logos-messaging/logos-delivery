...
proc runChannels(logos: LogosDelivery) {.async.} =
  ...
  ## The cipher is handed to the channel at creation and fixed for its life;
  ## `none` leaves the channel in plaintext.
  let secureCrypto = ChannelCrypto.init(aes.encrypt, aes.decrypt).valueOr:
    echo "failed to build the AES cipher: ", error
    return
  let toyCrypto = ChannelCrypto.init(xorc.encrypt, xorc.decrypt).valueOr:
    echo "failed to build the XOR cipher: ", error
    return

  for (channelId, contentTopic, encryption) in [
    (SecureChannel, ContentTopic("/example/1/secure/proto"), Opt.some(secureCrypto)),
    (ToyChannel, ContentTopic("/example/1/toy/proto"), Opt.some(toyCrypto)),
    (PlainChannel, ContentTopic("/example/1/plain/proto"), Opt.none(ChannelCrypto)),
  ]:
    logos.reliableChannelManager.createReliableChannel(
      channelId,
      contentTopic,
      SdsParticipantID("channel-encryption-example"),
      encryption,
    ).isOkOr:
